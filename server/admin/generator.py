...
def render_dcgm_csv(state: dict) -> str:
    lines = ["# AUTO-GENERATED by the Notify monitoring web admin.",
             "# Format: field, type, help"]
    for c in state.get("gpu", {}).get("counters", []):
        lines.append("%s, %s, %s" % (c["field"], c["type"], c["help"]))
    return "\n".join(lines) + "\n"


def render_env(state: dict) -> str:
    r = state.get("retention", {})
    monitor_dir = os.environ.get("MONITOR_DIR", "/opt/monitor")
    lines = [
        "# AUTO-GENERATED by the Notify monitoring web admin (retention / disk knobs).",
        "# docker-compose.yml reads these. Re-create services to apply: see web UI.",
        "MONITOR_DIR=%s" % monitor_dir,
        "PROM_RETENTION_TIME=%s" % r.get("prometheus_time", "15d"),
        "PROM_RETENTION_SIZE=%s" % r.get("prometheus_size", "0"),
        "LOG_MAX_SIZE=%s" % r.get("log_max_size", "10m"),
        "LOG_MAX_FILE=%s" % r.get("log_max_file", "3"),
        "GF_ADMIN_USER=%s" % state.get("grafana", {}).get("admin_user", "admin"),
        "GF_ADMIN_PASSWORD=%s" % state.get("grafana", {}).get("admin_password", "admin"),
    ]
    return "\n".join(lines) + "\n"
...
def write_all(state: dict, config_dir: Path, monitor_dir: Path) -> list[str]:
    ...
    env_path.write_text(render_env(state), encoding="utf-8")
