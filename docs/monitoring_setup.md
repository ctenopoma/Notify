...
📦 **この手順の実装が [`server/`](../server/) にあります。** 本書のスタックを動く docker compose として用意し、さらに **ブラウザ（`http://<host>:8088`）から監視対象・メトリクス・アラート・保持期間を設定し、設定ファイルを自動生成して compose を再起動できる Web 管理コンソール** を同梱しています。CPU/メモリ用の node-exporter も追加済みです。手で yml を書く代わりに UI で運用する場合は [`server/README.md`](../server/README.md) を参照してください（本書は仕組みの理解と、手動構築・トラブルシュート用のリファレンスとして残しています）。
